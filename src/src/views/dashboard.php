...
<head>
    ...
    <style>
        ...
        * {
        ...
        .swatch-batik-card:hover {
            transform: translateY(-2px);
            border-color: rgba(37, 99, 235, 0.3);
            box-shadow: 0 10px 15px -3px rgba(37, 99, 235, 0.2);
        }

        .swatch-batik-card.active {
            border-color: #2563eb;
...
if(file_exists($btn) && file_exists($win)) { 
    ...
    echo "<script>alert('ERROR BROWSER/HOSTING: File inc_chat_button.php atau inc_chat_window.php tidak ditemukan di dalam folder src/. Mohon pastikan Anda meng-uploadnya ke folder yang benar (src/), BUKAN di dalam src/views/ !');</script>"; 
} 
?>
    ...
    <!-- ═══ SIDEBAR ═══ -->
    <aside id="sidebar"
        class="w-[260px] frost dark:bg-[#0d1526]/90 dark:border-slate-800 border-r border-white/60 flex flex-col h-screen lg:relative shrink-0">
        <div class="p-6 flex items-center justify-between shrink-0">
            <div class="flex items-center gap-3">
                <div
                    class="w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center text-white shadow-lg shadow-blue-500/25">
                    <i class="fas fa-bolt text-lg"></i></div>
                <span class="font-black text-xl text-slate-900 dark:text-white tracking-tight">StepUp</span>
            </div>
            <button onclick="closeSB()" class="lg:hidden text-slate-400 hover:text-red-500 p-1"><i
                    class="fas fa-times"></i></button>
        </div>
        <nav class="flex-1 overflow-y-auto px-3 pb-3 space-y-1">
            <p class="text-[9px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest px-3 mb-2">
                Utama</p>
            <button onclick="sw('home')" id="nav-home" class="navi active"><i
                    class="fas fa-th-large w-5 tc"></i><span>Dashboard</span></button>
            <p
                class="text-[9px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest px-3 mb-2 mt-5">
                Materi</p>
        ...
        <div class="shrink-0 p-3 border-t border-slate-100 dark:border-slate-800/80">
            <div
                class="flex items-center gap-3 p-3 rounded-2xl bg-white/60 dark:bg-slate-800/50 border border-white/50 dark:border-slate-700/50 mb-2 overflow-hidden">
                <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white font-black shrink-0 shadow-lg shadow-blue-500/20 overflow-hidden relative"
                    id="sidebarPPBox">
                    <?php if (!empty($user['profile_pic']) && file_exists('../../' . $user['profile_pic'])): ?>
                        <img src="../../<?php echo htmlspecialchars($user['profile_pic']); ?>" alt="Profile"
                            class="w-full h-full object-cover" id="sidebarPP">
                    <?php else: ?>
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">
                        <?php echo htmlspecialchars(!empty($user['title']) ? $user['title'] : 'Siswa') ?></p>
                </div>
                <button onclick="toggleDark()"
                    class="w-8 h-8 rounded-lg text-slate-400 hover:text-blue-500 dark:hover:text-yellow-400 transition-colors flex items-center justify-center shrink-0"><i
                        class="fas fa-moon" id="mIcon"></i></button>
            </div>
            <button onclick="openSettings()" class="navi w-full mb-1"><i
                    class="fas fa-cog w-5 tc"></i><span>Pengaturan</span></button>
            <button onclick="doLogout()" class="navi w-full text-red-500 hover:!bg-red-50 dark:hover:!bg-red-900/20"><i
                    class="fas fa-sign-out-alt w-5 tc"></i><span>Sign Out</span></button>
        </div>
    </aside>
    ...
    <main class="flex-1 flex flex-col overflow-hidden">
        <header
            class="lg:hidden shrink-0 flex items-center justify-between px-5 py-4 frost dark:bg-[#0d1526]/90 border-b border-white/50 dark:border-slate-800">
            <button onclick="openSB()"
                class="w-10 h-10 rounded-xl bg-white/60 dark:bg-slate-800 flex items-center justify-center text-slate-500"><i
                    class="fas fa-bars"></i></button>
            <span class="font-black text-slate-900 dark:text-white">StepUp</span>
            <div class="flex items-center gap-2">
                <button onclick="openSettings()"
                    class="w-10 h-10 rounded-xl bg-white/60 dark:bg-slate-800 flex items-center justify-center overflow-hidden border border-white/40 dark:border-slate-700 shadow-sm"
                    id="mobilePPBox">
                    <?php if (!empty($user['profile_pic']) && file_exists('../../' . $user['profile_pic'])): ?>
                        <img src="../../<?php echo htmlspecialchars($user['profile_pic']); ?>" alt="Profile"
                            class="w-full h-full object-cover" id="mobilePP">
                    <?php else: ?>
                ...
                <div class="max-w-7xl mx-auto">
                    <div class="flex items-center justify-between mb-6">
                        ...
                        <div class="hidden lg:flex items-center gap-3">
                            <button onclick="toggleDark()"
                                class="w-11 h-11 rounded-2xl bg-white/80 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center text-slate-500 hover:text-blue-500 dark:text-slate-400 dark:hover:text-yellow-400 shadow-sm transition-all"><i
                                    class="fas fa-moon" id="mIcon2"></i></button>

                            <button onclick="openSettings()"
                                class="flex items-center gap-3 p-1.5 pr-4 rounded-2xl bg-white/80 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-blue-500 transition-all shadow-sm group">
                                <div class="w-8 h-8 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white font-black shrink-0 shadow-md overflow-hidden relative"
                                    id="navbarPPBox">
                                    <?php if (!empty($user['profile_pic']) && file_exists('../../' . $user['profile_pic'])): ?>
                                        <img src="../../<?php echo htmlspecialchars($user['profile_pic']); ?>" alt="Profile"
                                            class="w-full h-full object-cover" id="navbarPP">
                                    <?php else: ?>
                                        <span class="text-[10px]" id="navbarThumb"><?php echo $ini ?></span>
                                    <?php endif; ?>
                                </div>
                                <span
                                    class="text-xs font-black text-slate-600 dark:text-slate-300 uppercase tracking-widest group-hover:text-blue-500"><?php echo explode(' ', $user['full_name'])[0] ?></span>
                            </button>

                            <button onclick="openSettings()"
                                class="w-11 h-11 rounded-2xl bg-white/80 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center text-slate-500 hover:text-blue-500 shadow-sm transition-all"><i
                                    class="fas fa-cog"></i></button>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 xl:grid-cols-12 gap-5">
                        <!-- Calendar -->
                        <div
                            class="xl:col-span-7 frost cal-card rounded-3xl overflow-hidden border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col">
                            <div class="p-6 border-b border-slate-50 dark:border-slate-800/50">
                                <div class="flex items-center justify-between">
                                    <div>
                                        <h2 class="font-black text-slate-900 dark:text-white text-lg tracking-tight">
                                            Kalender Akademik</h2>
                                        <p
                                            class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">
                                            Jadwal & Agenda Pribadi</p>
                                    </div>
                                    <div
                                        class="flex items-center gap-2 bg-slate-50 dark:bg-slate-900/50 p-1.5 rounded-2xl border border-slate-100 dark:border-slate-800">
                                        <button onclick="chM(-1)"
                                            class="w-8 h-8 rounded-xl bg-white dark:bg-slate-800 tc flex items-center justify-center text-slate-400 hover:text-blue-600 shadow-sm transition-all"><i
                                                class="fas fa-chevron-left"></i></button>
                                        <span id="calLbl"
                                            class="px-2 font-black text-[10px] uppercase tracking-widest text-slate-700 dark:text-slate-200 min-w-[120px] text-center"></span>
                                        <button onclick="chM(1)"
                                            class="w-8 h-8 rounded-xl bg-white dark:bg-slate-800 tc flex items-center justify-center text-slate-400 hover:text-blue-600 shadow-sm transition-all"><i
                            ...
                            <div
                                class="mt-auto p-5 bg-slate-50/50 dark:bg-slate-900/30 border-t border-slate-100 dark:border-slate-800/50">
                                <div class="flex items-center justify-between mb-4">
                                    <h3
                                        class="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-[0.2em]">
                                        Agenda Bulan Ini</h3>
                                    <button onclick="addAgenda()"
                                        class="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-[9px] font-black uppercase tracking-widest transition-all shadow-lg shadow-blue-500/20 flex items-center gap-2">
                                        <i class="fas fa-plus"></i> Tambah Acara
                                    </button>
                                </div>
                                <div id="agendaList"
                                    class="space-y-2 max-h-[150px] overflow-y-auto custom-scrollbar pr-2">
                                    <!-- Populated by JS -->
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="xl:col-span-5 flex flex-col gap-4">
                            <div class="relative overflow-hidden rounded-[3rem] p-12 text-white shadow-2xl flex items-center min-h-[220px] group/quote"
                                style="background:linear-gradient(135deg,#6366f1,#3b82f6)">
                                <div
                                    class="absolute top-0 right-0 w-80 h-80 bg-white/10 rounded-full -mr-40 -mt-40 blur-[100px] transition-transform duration-1000 group-hover/quote:scale-125">
                                </div>
                                <div
                                    class="absolute bottom-0 left-0 w-60 h-60 bg-blue-400/10 rounded-full -ml-30 -mb-30 blur-[80px]">
                                </div>

                                <div class="relative z-10 w-full">
                                    <i
                                        class="fas fa-quote-left text-4xl opacity-30 mb-6 block group-hover/quote:-translate-y-1 transition-transform"></i>
                                    <p
                                        class="text-2xl md:text-3xl font-black italic mb-6 tracking-tight leading-[1.15] drop-shadow-xl text-white/95">
                                        "<?php echo htmlspecialchars($quote) ?>"</p>
                                    <div class="flex items-center gap-3">
                                        <span class="w-12 h-[3px] bg-white/40 rounded-full"></span>
                                        <p class="text-[11px] font-black uppercase tracking-[0.4em] opacity-80">Boost
                                            Energi Hari Ini</p>
                                    </div>
                                </div>
                                ...
                                <div
                                    class="absolute bottom-[-20px] right-[-20px] lg:right-4 lg:bottom-4 group-hover/quote:rotate-[25deg] transition-all duration-700 pointer-events-none">
                                    <i
                                        class="fas fa-graduation-cap text-[180px] lg:text-[220px] text-white/10 rotate-[15deg]"></i>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <!-- Streak Card -->
                                <div onclick="showStreakDetails()"
                                    class="frost dark:bg-[#0d1526]/80 rounded-[2.5rem] p-6 border border-white/50 dark:border-slate-800 shadow-sm transition-all hover:shadow-xl hover:-translate-y-1 active:scale-95 cursor-pointer group/streak relative overflow-hidden">
                                    <div
                                        class="absolute -right-4 -top-4 w-24 h-24 bg-orange-500/5 rounded-full blur-2xl group-hover/streak:bg-orange-500/10 transition-colors">
                                    </div>
                                    <div
                                        class="w-12 h-12 rounded-2xl bg-orange-100 dark:bg-orange-900/30 flex items-center justify-center mb-4 text-orange-500 text-xl group-hover/streak:scale-110 transition shadow-inner">
                                        <i class="fas fa-fire"></i>
                                    </div>
                                    <p class="text-4xl font-black text-slate-900 dark:text-white mb-1">
                                        <?php echo $current_streak ?></p>
                                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">HARI
                                        STREAK</p>
                                </div>

                                <!-- Progress Card -->
                                <div onclick="showProgressDetails()"
                                    class="frost dark:bg-[#0d1526]/80 rounded-[2.5rem] p-6 border border-white/50 dark:border-slate-800 shadow-sm transition-all hover:shadow-xl hover:-translate-y-1 active:scale-95 cursor-pointer group/prog relative overflow-hidden">
                                    <div
                                        class="absolute -right-4 -top-4 w-24 h-24 bg-blue-500/5 rounded-full blur-2xl group-hover/prog:bg-blue-500/10 transition-colors">
                                    </div>
                                    <div
                                        class="w-12 h-12 rounded-2xl bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center mb-4 text-blue-600 text-xl group-hover/prog:scale-110 transition shadow-inner">
                                        <i class="fas fa-chart-line"></i>
                                    </div>
                                    <p class="text-4xl font-black text-slate-900 dark:text-white mb-1">
                                        <?php echo $total_progress ?>%</p>
                                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">TOTAL
                                        PENGUASAAN</p>
                                </div>
                            </div>
                            ...
                            <!-- TODO -->
                            <div
                                class="flex-1 frost dark:bg-[#0d1526]/80 rounded-2xl p-5 border border-white/50 dark:border-slate-800 shadow-sm">
                                <div class="flex items-center justify-between mb-4">
                                    <h3 class="font-black text-slate-900 dark:text-white">Tugas Hari Ini</h3>
                                    <span class="pill bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300"
                                        id="todoCnt">0 tersisa</span>
                                </div>
                                <!-- Add form -->
                                <form onsubmit="addTodo(event)" class="flex gap-2 mb-3">
                                    <input id="todoInput" type="text" placeholder="Tambah tugas baru..."
                                        class="inp flex-1 !py-2.5 text-sm">
                                    <button type="submit"
                                        class="w-10 h-10 bg-blue-600 hover:bg-blue-700 text-white rounded-xl flex items-center justify-center shadow-md shadow-blue-500/20 shrink-0 transition"><i
                    ...
                    <div class="mt-10 mb-10 w-full">
                        <div class="frost rounded-[3rem] border border-white/70 dark:border-slate-800 shadow-md p-8">
                            <div class="mb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
                                <div class="flex-1">
                                    <h3
                                        class="text-3xl font-black text-slate-900 dark:text-white tracking-tighter leading-none mb-3">
                                        Aktivitas Terkini</h3>
                                    <p class="text-sm font-semibold text-slate-400">Log belajar dan pencapaian
                                        terbarumu.</p>
                                </div>
                                <button onclick="openActivityHistory()"
                                    class="px-6 py-2.5 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-blue-100 dark:border-blue-900/10 hover:bg-blue-600 hover:text-white transition-all shadow-sm shrink-0">
                                    Lihat Semua
                                </button>
                            </div>
                            ...
                            <div class="w-full">
                                ...
                                if (empty($top_activities)): ?>
                                    <div
                                        class="w-full p-12 bg-white/60 dark:bg-slate-800/30 rounded-[3rem] border-2 border-dashed border-slate-200 dark:border-slate-800 flex flex-col md:flex-row items-center justify-center gap-8 text-center md:text-left transition-all hover:border-blue-300">
                                        <div
                                            class="w-24 h-24 bg-white dark:bg-slate-800 rounded-[2rem] flex items-center justify-center shadow-xl text-slate-200 dark:text-slate-700">
                                            <i class="fas fa-history text-5xl"></i>
                                        </div>
                                        <div class="max-w-md">
                                            <h3
                                                class="font-black text-slate-800 dark:text-white text-xl tracking-tight mb-2">
                                                Belum Ada Jejak Belajar</h3>
                                            <p
                                                class="text-[13px] font-bold text-slate-400 leading-relaxed uppercase tracking-wider">
                                                Materi sedang menunggu untuk dipelajari. Selesaikan kuis atau baca modul
                                                untuk melihat progresmu di sini!</p>
                                        </div>
                                    </div>
                                <?php else: ?>
                                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                                        <?php foreach ($top_activities as $act):
                                            ...
                                            ?>
                                            <div
                                                class="bg-white/90 dark:bg-slate-800/60 backdrop-blur-md p-6 rounded-[2.5rem] shadow-md border border-white/80 dark:border-slate-700 flex items-center justify-between group hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                                                <div class="flex items-center space-x-6">
                                                    <div
                                                        class="w-14 h-14 rounded-2xl bg-<?php echo $color ?>-50 dark:bg-<?php echo $color ?>-900/20 text-<?php echo $color ?>-600 dark:text-<?php echo $color ?>-400 flex items-center justify-center text-xl shadow-inner group-hover:scale-110 duration-300 transition-transform">
                                                        <i class="fas <?php echo $icon ?>"></i>
                                                    </div>
                                                    <div>
                                                        <p
                                                            class="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] mb-1">
                                                            <?php echo $act['action'] ?></p>
                                                        <h4
                                                            class="font-black text-slate-800 dark:text-white tracking-tight leading-none text-sm truncate max-w-[120px]">
                                                            <?php echo htmlspecialchars($act['mod_name']) ?></h4>
                                                        <?php if ($isQuiz): ?>
                                                            <span
                                                                class="inline-block mt-2 px-2 py-0.5 bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 text-[9px] font-black rounded-lg border border-amber-100 dark:border-amber-800">SKOR:
                                                                <?php echo $act['details'] ?>%</span>
                                                        <?php endif; ?>
                                                    </div>
                                                </div>
                                                <div class="text-right">
                                                    <p
                                                        class="text-[9px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest mb-1">
                                                        <?php echo date('d M', strtotime($act['time_ref'])) ?></p>
                                                    <p
                                                        class="text-lg font-black text-slate-800 dark:text-white tabular-nums tracking-tighter">
                                                        <?php echo date('H:i', strtotime($act['time_ref'])) ?></p>
                                                </div>
                                            </div>
                                        <?php endforeach; ?>
                                    </div>
            ...
            <?php foreach ($subjects as $sl => $s): ?>
                <div id="tab-<?php echo $sl ?>" class="tp pb-20">
                    <div class="max-w-7xl mx-auto py-10">
                        ...
                        <div class="relative overflow-hidden rounded-[3rem] p-10 md:p-14 text-white mb-10 shadow-2xl"
                            style="background: linear-gradient(135deg, <?php echo $s['color'] ?> 0%, <?php echo $s['color'] ?>cc 100%);">
                            <div class="relative z-10 flex flex-col md:flex-row items-center md:justify-between gap-8">
                                <div class="text-center md:text-left">
                                    <div
                                        class="pill bg-white/30 border border-white/40 text-white mb-4 backdrop-blur-md px-4 py-1 text-xs">
                                        Kurikulum 2026</div>
                                    <h2
                                        class="text-4xl md:text-6xl font-black tracking-tighter uppercase mb-4 leading-none drop-shadow-xl">
                                        <?php echo htmlspecialchars($s['title']) ?></h2>
                                    <p
                                        class="text-sm md:text-base font-bold opacity-90 tracking-wide italic max-w-xl leading-relaxed">
                                        <?php echo htmlspecialchars($s['desc'] ?: 'Pelajari ' . $s['title'] . ' dengan materi eksklusif dan interaktif di platform StepUp Learning.') ?>
                                    </p>
                                </div>
                                <div
                                    class="bg-white/10 dark:bg-black/20 backdrop-blur-3xl text-white rounded-2xl p-8 text-center shadow-lg min-w-[180px] group/score relative border border-white/25 overflow-hidden transition-transform">
                                    <div class="relative z-10 flex flex-col items-center">
                                        <div class="flex items-end gap-1">
                                            <span
                                                class="text-5xl font-black leading-none"><?php echo $s['progress'] ?></span>
                                            <span class="text-xl font-black opacity-60 mb-1">%</span>
                                        </div>
                                        <div class="text-[10px] font-black uppercase tracking-[0.2em] opacity-80 mt-2">
                                            PENGUASAAN</div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <i
                                class="fas fa-graduation-cap absolute bottom-[-15px] right-12 text-[140px] opacity-10 rotate-[20deg] pointer-events-none"></i>
                        </div>
                        ...
                        <div class="flex gap-5">
                            ...
                            <div class="flex-1 min-w-0">
                                <?php if (empty($s['topics'])): ?>
                                    <div
                                        class="text-center p-20 frost rounded-[2.5rem] border-2 border-dashed border-slate-200/50">
                                        <p class="font-black text-slate-400 uppercase text-sm tracking-widest">Materi sedang
                                            diracik...</p>
                                    </div>
                                <?php else: ?>
                                    <div class="space-y-4">
                                        <?php foreach ($s['topics'] as $ts => $top):
                                            ...
                                            ?>
                                            <div class="group">
                                                <button onclick="tgAcc('<?php echo $sl . '-' . $ts ?>')"
                                                    class="w-full flex items-center justify-between p-7 frost rounded-[2.5rem] hover:bg-white transition-all shadow-sm group-hover:shadow-md border border-white/60 dark:border-slate-800">
                                                    <div class="flex items-center gap-6">
                                                        <div
                                                            class="w-14 h-14 rounded-2xl bg-blue-50 dark:bg-blue-900/40 text-blue-600 flex items-center justify-center text-xl shadow-inner group-hover:scale-110 transition shrink-0">
                                                            <i class="fas fa-folder<?php echo $is_topic_done ? '-open' : '' ?>"></i>
                                                        </div>
                                                        <div class="text-left">
                                                            <h4
                                                                class="font-black text-slate-900 dark:text-white text-xl tracking-tighter leading-none">
                                                                <?php echo htmlspecialchars($top['title']) ?></h4>
                                                            <p
                                                                class="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-2">
                                                                <?php echo count($top['modules']) ?> MODUL PEMBELAJARAN</p>
                                                        </div>
                                                    </div>
                                                    <div class="flex items-center gap-3">
                                                        <?php if ($is_topic_done): ?>
                                                            <a href="certificate.php?topic=<?php echo urlencode($ts) ?>"
                                                                class="pill bg-emerald-500 text-white border-none px-4 py-2 scale-110 hover:bg-emerald-600 transition shadow-lg shadow-emerald-500/30 flex items-center gap-2">
                                                                <i class="fas fa-certificate"></i> CETAK SERTIFIKAT
                                                            </a>
                                                        <?php endif; ?>
                                                        <i id="chv-<?php echo $sl . '-' . $ts ?>"
                                                            class="fas fa-chevron-down text-slate-300 aci transition-transform duration-300"></i>
                                                    </div>
                                                </button>
                                                <div id="acc-<?php echo $sl . '-' . $ts ?>"
                                                    class="acb mt-5 pl-4 border-l-4 border-blue-500/10 ml-7">
                                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 py-2">
                                                        <?php foreach ($top['modules'] as $ms => $mod):
                                                            $is_done = $mod['is_completed'];
                                                            ?>
                                                            <div
                                                                class="p-8 rounded-[2.5rem] border-2 transition-all hover:shadow-2xl hover:-translate-y-1 relative group/mod flex flex-col h-full <?php echo $is_done ? 'bg-emerald-50/50 border-emerald-100 dark:bg-emerald-900/10 dark:border-emerald-900/30' : 'bg-white/60 dark:bg-slate-800/40 border-slate-100 dark:border-slate-800' ?>">
                                                                <div
                                                                    class="pill <?php echo $is_done ? 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600' : 'bg-blue-100 dark:bg-blue-900/40 text-blue-600' ?> mb-6 px-4 py-1.5 text-[10px] w-fit">
                                                                    MODUL</div>
                                                                <h5
                                                                    class="font-black text-xl text-slate-800 dark:text-white mb-10 leading-snug tracking-tighter">
                                                                    <?php echo htmlspecialchars($mod['title']) ?></h5>

                                                                <div class="mt-auto">
                                                                    <a href="module.php?subject=<?php echo urlencode($sl) ?>&topic=<?php echo urlencode($ts) ?>&module=<?php echo urlencode($ms) ?>"
                                                                        class="w-full py-5 <?php echo $is_done ? 'bg-emerald-500 hover:bg-emerald-600 shadow-emerald-500/20' : 'bg-blue-600 hover:bg-blue-700 shadow-blue-500/20' ?> text-white rounded-2xl font-black text-center text-[11px] uppercase tracking-[.15em] transition flex items-center justify-center gap-3 shadow-lg active:scale-95">
                                                                ...
                                                                <?php if ($is_done): ?>
                                                                    <div
                                                                        class="absolute -top-3 -right-3 w-12 h-12 bg-emerald-500 text-white rounded-full flex items-center justify-center shadow-lg border-4 border-white dark:border-slate-900 animate-bounce">
                                                    ...
                                                    <?php if ($is_topic_done): ?>
                                                        <div
                                                            class="mt-8 p-8 rounded-[2.5rem] bg-gradient-to-r from-yellow-400 to-amber-500 text-white flex flex-col sm:flex-row items-center justify-between gap-6 shadow-2xl shadow-yellow-500/30 border-2 border-white/20">
                                                            <div class="flex items-center gap-6">
                                                                <div
                                                                    class="w-20 h-20 bg-white/20 rounded-[1.5rem] flex items-center justify-center text-4xl backdrop-blur-md shadow-inner">
                                                                    <i class="fas fa-trophy"></i></div>
                                                                <div class="text-center sm:text-left">
                                                                    <p class="font-black text-2xl uppercase tracking-tighter">Topik
                                                                        Dikuasai!</p>
                                                                    <p
                                                                        class="text-xs font-bold opacity-90 uppercase tracking-widest mt-1">
                                                                        Sertifikat digital topik <?php echo $top['title'] ?> telah
                                                                        terbit.</p>
                                                                </div>
                                                            </div>
                                                            <a href="certificate.php?topic=<?php echo urlencode($ts) ?>&subject=<?php echo urlencode($sl) ?>&type=topic"
                                                                class="w-full sm:w-auto px-10 py-4 bg-white text-amber-600 rounded-2xl font-black text-xs uppercase tracking-widest hover:scale-110 transition shadow-xl active:scale-95 shrink-0 flex items-center justify-center gap-3">
                            ...
                            <div class="w-80 shrink-0 hidden xl:flex flex-col gap-4">
                                ...
                                <div
                                    class="frost rounded-3xl p-6 border border-white/60 dark:border-slate-700 shadow-sm transition-all hover:shadow-xl hover:shadow-blue-500/5">
                                    <div class="flex items-center justify-between mb-4">
                                        <span
                                            class="text-xs font-black text-slate-400 uppercase tracking-widest">Kalender</span>
                                        <span class="text-xs font-extrabold text-blue-600"
                                            id="miniCalLbl-<?php echo $sl ?>"></span>
                                    </div>
                                    <div class="grid grid-cols-7 text-center mb-2">
                                        <?php foreach (['M', 'S', 'S', 'R', 'K', 'J', 'S'] as $d): ?>
                                            <div class="text-[10px] font-black text-slate-300 uppercase"><?php echo $d ?></div>
                                        <?php endforeach; ?>
                                    </div>
                                    ...
                                    <div class="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800/50">
                                        <div class="flex items-center justify-between mb-3">
                                            <h3
                                                class="text-[9px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-[0.2em]">
                                                Agenda Bulan Ini</h3>
                                            <button onclick="addAgenda()"
                                                class="px-2 py-1 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-[8px] font-black uppercase tracking-widest transition-all shadow-md shadow-blue-500/20 flex items-center gap-1">
                                                <i class="fas fa-plus"></i> Tambah
                                            </button>
                                        </div>
                                        <div id="miniAgendaList-<?php echo $sl ?>"
                                            class="space-y-2 max-h-[120px] overflow-y-auto custom-scrollbar pr-1">
                                            <!-- Populated by JS -->
                                        </div>
                                    </div>
                                </div>
                                <!-- Mini Quote -->
                                <div class="relative overflow-hidden rounded-[3rem] p-8 text-white shadow-2xl group/quote transition-all hover:scale-[1.02]"
                                    style="background:linear-gradient(135deg,<?php echo $s['color'] ?>,<?php echo $s['color'] ?>cc)">
                                    <div class="relative z-10">
                                        <i
                                            class="fas fa-quote-left mb-4 text-white/40 text-2xl block group-hover/quote:-translate-y-1 transition-transform"></i>
                                        <p class="italic text-lg text-white leading-relaxed font-bold tracking-tight">
                                            &ldquo;<?php echo htmlspecialchars(substr($quote, 0, 120)) ?>&rdquo;</p>
                                    </div>
                                    <!-- Toga Icon Decoration -->
                                    <i
                                        class="fas fa-graduation-cap absolute bottom-[-15px] right-2 text-[110px] opacity-10 rotate-[20deg] pointer-events-none"></i>
                                </div>
                                <!-- Mini Todo -->
                                <div
                                    class="frost dark:bg-[#0d1526]/80 rounded-[3rem] p-8 border border-white/50 dark:border-slate-800 shadow-sm flex-1 transition-all hover:shadow-xl hover:shadow-blue-500/5">
                                    <div class="flex items-center justify-between mb-6">
                                        <p class="text-xs font-black text-slate-400 uppercase tracking-widest">Tugas
                                            Berjalan</p>
                                        <button onclick="addQuickTodo()"
                                            class="w-10 h-10 bg-blue-600 text-white rounded-2xl flex items-center justify-center text-xs shadow-xl shadow-blue-500/30 hover:scale-110 active:scale-90 transition"><i
                                                class="fas fa-plus"></i></button>
                                    </div>
                                    <div id="miniTodo-<?php echo $s['id'] ?>"
                                        class="mini-todo-list space-y-3.5 max-h-[400px] overflow-y-auto pr-1 custom-scrollbar">
                                    </div>
                                </div>
                            </div>
                        </div> <!-- End .flex gap-5 -->
    ...
    <div id="sModal" class="moverlay" onclick="if(event.target===this)closeSettings()">
        <div class="relative w-full max-w-lg bg-white/80 dark:bg-[#0a1128]/90 rounded-[3.5rem] shadow-2xl overflow-hidden border border-white/20 dark:border-blue-900/20 backdrop-blur-2xl"
            style="animation:swal-modern-in .6s cubic-bezier(0.175, 0.885, 0.32, 1.275) both">
            <!-- Header -->
            <div class="p-10 pb-6">
                <button onclick="closeSettings()"
                    class="absolute top-8 right-8 w-10 h-10 bg-slate-100 hover:bg-red-500 hover:text-white dark:bg-slate-800 dark:hover:bg-red-500 rounded-2xl flex items-center justify-center text-slate-400 transition-all shadow-sm"><i
                        class="fas fa-times"></i></button>
                <div class="flex items-center gap-6 mb-8">
                    <div
                        class="w-20 h-20 rounded-[2rem] bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white font-black text-3xl shadow-xl shadow-blue-500/30">
                        <?php echo $ini ?></div>
                    <div>
            ...
            <div class="p-10 pt-4 overflow-y-auto max-h-[60vh] custom-scrollbar">
                ...
                <div id="s-profile" class="stpane">
                    <div class="space-y-6">
                        ...
                        <div
                            ...
                            <div class="w-24 h-24 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white text-3xl font-black shadow-xl mb-4 relative group overflow-hidden"
                                id="ppPreviewBox">
                                <?php if (!empty($user['profile_pic']) && file_exists('../../' . $user['profile_pic'])): ?>
                                    <img src="../../<?php echo htmlspecialchars($user['profile_pic']); ?>" id="ppPreview"
                ...
                <div id="s-appearance" class="stpane hidden">
                    <div class="space-y-6">
                        ...
                        <div
                            class="flex items-center justify-between p-5 bg-slate-50 dark:bg-slate-800/40 rounded-3xl border border-slate-100 dark:border-slate-800/50">
                        ...
                        <div>
                            <div class="flex items-center justify-between mb-4 px-2">
                                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Warna
                                    Suasana</label>
                                <button onclick="resetBg()"
                                    class="text-[10px] font-black text-blue-500 hover:text-blue-600 uppercase tracking-widest">Reset
                                    Default</button>
                            </div>
                            ...
                            <div class="space-y-3 pt-1 mb-4">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    ...
                                    <button onclick="setBg('batik','assets/img/mega.jpg')"
                                        ...
                                        <span
                                            class="text-white text-[10px] font-black uppercase tracking-widest">Megamendung</span>
                                        <?php if ($pref['bg_value'] === 'assets/img/mega.jpg'): ?><i
                                                class="fas fa-check-circle text-white"></i><?php endif; ?>
                                    </button>
                                </div>
                            </div>
                            ...
                            <div class="mt-6 mb-6">
                                <div class="flex items-center justify-between mb-4 px-2">
                                    <label
                                        class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Transparansi
                                        Kaca</label>
                                    <span class="text-[10px] font-bold text-blue-500"
                                        id="opacityValLabel"><?php echo $pref['glass_opacity'] ?>%</span>
                                </div>
                                <div
                                    class="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-3xl border border-slate-100 dark:border-slate-800">
                                    <div class="grid grid-cols-5 gap-2" id="opacityControls">
                                        <?php foreach ([5, 25, 50, 75, 100] as $ov): ?>
                                            <button onclick="setGlassOpacity(<?php echo $ov ?>)" id="gop-<?php echo $ov ?>"
                            ...
                            <div class="pt-2">
                                <label
                                    ...
                                    Background</label>
                                <label
                                    class="group block relative p-6 bg-slate-50 dark:bg-slate-800/40 rounded-2xl border-2 border-dashed border-slate-100 dark:border-slate-800 hover:border-blue-400 dark:hover:border-blue-500 transition-all cursor-pointer overflow-hidden">
                                    <div class="flex items-center gap-4 relative z-10">
                                        <div
                                            class="w-12 h-12 bg-white dark:bg-slate-700 rounded-xl flex items-center justify-center text-slate-400 group-hover:text-blue-500 transition-colors shadow-sm">
        ...
        <div id="activityModal" class="moverlay" onclick="if(event.target===this)closeActivityHistory()">
            <div class="relative w-full max-w-2xl bg-white dark:bg-[#0d1526] rounded-[3rem] shadow-2xl overflow-hidden border border-slate-100 dark:border-slate-800"
                style="animation:fu .3s ease both">
                <div class="p-8 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
                    <div>
                        <h2 class="text-2xl font-black text-slate-800 dark:text-white tracking-tighter">Riwayat
                            Aktivitas</h2>
                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Seluruh log
                            belajar kamu</p>
                    </div>
                    <button onclick="closeActivityHistory()"
                        class="w-10 h-10 bg-slate-100 dark:bg-slate-800 rounded-2xl flex items-center justify-center text-slate-500 hover:text-red-500 transition-colors"><i
                            class="fas fa-times"></i></button>
                </div>
                <div
                    class="p-8 max-h-[70vh] overflow-y-auto custom-scrollbar space-y-4 bg-slate-50/50 dark:bg-slate-900/50">
                    <?php if (!empty($user_activities)): ?>
                        <?php foreach ($user_activities as $act):
                            ...
                            ?>
                            <div
                                class="flex items-center justify-between p-6 bg-white dark:bg-slate-800/40 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm">
                                <div class="flex items-center space-x-5">
                                    <div
                                        class="w-12 h-12 rounded-xl bg-<?php echo $color ?>-100 dark:bg-<?php echo $color ?>-900/30 text-<?php echo $color ?>-600 dark:text-<?php echo $color ?>-400 flex items-center justify-center text-xl">
                                        <i class="fas <?php echo $icon ?>"></i>
                                    </div>
                                    <div>
                                        <h4 class="font-black text-slate-800 dark:text-white text-sm leading-none mb-1.5">
                                            <?php echo htmlspecialchars($act['mod_name']) ?></h4>
                                        <div class="flex items-center gap-2">
                                            ...
                                            <?php if ($isQuiz): ?>
                                                <span
                                                    class="px-1.5 py-0.5 bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 text-[8px] font-black rounded-md border border-amber-100 dark:border-amber-800"><?php echo $act['details'] ?>%</span>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                </div>
                                <div class="text-right">
                                    <p
                                        class="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">
                                        <?php echo date('d M Y', strtotime($act['time_ref'])) ?></p>
                                    <p class="text-lg font-black text-slate-800 dark:text-white tabular-nums tracking-tighter">
                                        <?php echo date('H:i', strtotime($act['time_ref'])) ?></p>
                                </div>
                            </div>
                        <?php endforeach; ?>
    ...
    `).join('');
            ...
            function showStreakDetails() {
                const dates = <?php echo json_encode($streak_days ?? [], JSON_HEX_TAG | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_HEX_APOS) ?>;
    ...
    `;
        ...
        `;
            ...
            function showProgressDetails() {
                const subjects = <?php echo json_encode($subjects ?? [], JSON_HEX_TAG | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_HEX_APOS) ?>;
    ...
    `;
        ...
        `;
            ...
            window.openActivityHistory = function() {
                ...
                if (modal) {
                    // Portal modal ke <body> agar tidak terperangkap di dalam sModal (display:none)
                ...
                // Fallback to Swal if modal not found
                const activities = <?php echo json_encode($user_activities ?? [], JSON_HEX_TAG | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_HEX_APOS) ?>;
                if (!activities || activities.length === 0) {
                    Swal.fire({
                        ...getModernConfig(),
                        title: 'Riwayat Aktivitas',
                        iconHtml: '<div class="text-blue-500 scale-125"><i class="fas fa-history"></i></div>',
                        html: `
                            <div class="text-center py-12 bg-slate-50 dark:bg-slate-900/50 rounded-[2rem] border-2 border-dashed border-slate-200 dark:border-slate-800">
                                <i class="fas fa-history text-4xl text-slate-200 dark:text-slate-700 mb-4 block"></i>
                                <p class="text-slate-400 font-bold italic">Belum ada aktivitas belajar tercatat.</p>
                                <p class="text-slate-300 text-xs mt-2">Mulailah membaca modul atau mengerjakan kuis!</p>
                            </div>`,
                        confirmButtonText: 'TUTUP',
                        customClass: { htmlContainer: 'text-left', icon: 'border-none bg-blue-50 dark:bg-blue-950/30' }
                    });
                    return;
                }

                const iconMap = { quiz: 'fa-certificate', progress: 'fa-book-reader' };
                const colorMap = { quiz: 'amber', progress: 'blue' };
                const labelMap = { quiz: 'Menyelesaikan Kuis', progress: 'Membaca Modul' };

                let html = `<div class="text-left p-2">
                    <div class="flex items-center justify-between mb-6 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-950/20 dark:to-indigo-950/20 p-6 rounded-[2rem] border border-blue-100/50 dark:border-blue-800/30">
                        <div>
                            <p class="text-[10px] font-black text-blue-500 uppercase tracking-[0.2em] mb-1">TOTAL AKTIVITAS</p>
                            <p class="text-4xl font-black text-slate-900 dark:text-white">${activities.length} <span class="text-base opacity-50">Kegiatan</span></p>
                        </div>
                        <div class="w-16 h-16 bg-blue-500 text-white rounded-2xl flex items-center justify-center text-2xl shadow-xl shadow-blue-500/30">
                            <i class="fas fa-chart-bar"></i>
                        </div>
                    </div>
                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.25em] mb-4 flex items-center gap-2">
                        <i class="fas fa-history"></i> LOG AKTIVITAS LENGKAP
                    </p>
                    <div class="space-y-3 max-h-[420px] overflow-y-auto pr-2 custom-scrollbar">`;

                activities.forEach(act => {
                    const isQuiz = act.type === 'quiz';
                    const icon = iconMap[act.type] || 'fa-circle';
                    const color = colorMap[act.type] || 'slate';
                    const label = act.action || labelMap[act.type] || 'Aktivitas';
                    const timeStr = act.time_ref ? new Date(act.time_ref).toLocaleString('id-ID', {
                        weekday: 'short', day: 'numeric', month: 'short', year: 'numeric',
                        hour: '2-digit', minute: '2-digit'
                    }) : '';

                    html += `
                        <div class="flex items-center gap-4 p-4 bg-white dark:bg-slate-800/40 rounded-[1.5rem] border border-slate-100 dark:border-slate-800 hover:border-${color}-200 dark:hover:border-${color}-900/50 transition-all hover:shadow-md group">
                            <div class="w-12 h-12 rounded-[1.2rem] bg-${color}-50 dark:bg-${color}-900/20 text-${color}-600 dark:text-${color}-400 flex items-center justify-center shrink-0 shadow-inner group-hover:scale-110 transition-transform">
                                <i class="fas ${icon} text-lg"></i>
                            </div>
                            <div class="flex-1 min-w-0">
                                <p class="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">${label}</p>
                                <p class="font-black text-slate-800 dark:text-slate-200 text-sm leading-tight truncate">${act.mod_name || act.details || '-'}</p>
                                ${isQuiz && act.details ? `<p class="text-[10px] font-bold text-${color}-500 mt-1">SKOR: ${act.details}%</p>` : ''}
                            </div>
