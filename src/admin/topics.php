    ...
    <main class="flex-1 flex flex-col h-full relative overflow-hidden">
        ...
        <header class="p-8 pb-4 flex flex-col md:flex-row justify-between items-start md:items-center bg-white/30 dark:bg-[#020617]/30 backdrop-blur-xl border-b border-blue-50/50 dark:border-blue-900/20 z-10">
            <div class="flex items-center space-x-6">
                <a href="subjects.php" class="w-12 h-12 bg-white/50 dark:bg-blue-900/20 text-slate-500 dark:text-blue-400 rounded-2xl flex items-center justify-center hover:bg-blue-600 hover:text-white transition-all transform active:scale-95 border border-blue-100 dark:border-blue-900/30">
                    <i class="fas fa-chevron-left text-sm"></i>
                </a>
                <div>
                    <div class="flex items-center space-x-3 mb-2">
                        <span class="px-3 py-1 bg-blue-600 text-[10px] font-black text-white rounded-lg uppercase tracking-widest"><?php echo htmlspecialchars($subject['title']); ?></span>
                        <span class="text-slate-400 dark:text-slate-500 text-[10px] font-black uppercase tracking-widest italic">/ Topics List</span>
                    </div>
                    <h1 class="text-4xl font-black text-slate-900 dark:text-white tracking-tighter leading-none mb-1">Topik Pembelajaran</h1>
                    <p class="text-slate-500 dark:text-slate-400 font-medium text-xs italic">Kelola struktur bab dan alur belajar siswa.</p>
                </div>
            </div>
            ...
            <div class="flex-1 overflow-y-auto p-8 custom-scrollbar scroll-smooth">
                <div class="max-w-7xl mx-auto space-y-8 pb-32">
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        <?php if (empty($topics)): ?>
                            <div class="col-span-full py-32 bg-white/50 dark:bg-[#0a1128]/50 backdrop-blur-xl rounded-[4rem] text-center border border-dashed border-blue-200 dark:border-blue-900/20">
                                ...
                                <h3 class="text-3xl font-black text-slate-800 dark:text-white mb-4 tracking-tighter">Topik Kosong</h3>
                                <p class="text-slate-400 font-bold uppercase tracking-[0.2em] text-[10px]">Belum ada bab yang dibuat untuk kurikulum ini.</p>
                            </div>
                        <?php endif; ?>

                        <?php foreach ($topics as $idx => $t): ?>
                            <div class="group relative bg-white/70 dark:bg-[#0a1128]/70 backdrop-blur-xl rounded-[3.5rem] p-8 border border-blue-50/50 dark:border-blue-900/20 hover:border-blue-500/50 transition-all duration-700 shadow-sm hover:shadow-2xl hover:shadow-blue-500/10 flex flex-col items-center text-center overflow-hidden h-full hover:-translate-y-2">
                                <div class="absolute inset-0 bg-gradient-to-br from-blue-600/[0.03] to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>
                                
                                <div class="w-20 h-20 bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-600/10 dark:to-blue-600/20 text-blue-600 dark:text-blue-400 rounded-3xl flex items-center justify-center text-2xl font-black border border-blue-100 dark:border-blue-900/20 transition-all group-hover:scale-110 group-hover:rotate-12 mb-8 relative z-10 shadow-inner">
                                    <?php echo str_pad($idx + 1, 2, '0', STR_PAD_LEFT); ?>
                                </div>

                                <div class="relative z-10 flex-1 flex flex-col w-full mb-10">
                                    <h4 class="text-2xl font-black text-slate-900 dark:text-white tracking-tighter group-hover:text-blue-600 transition-colors line-clamp-2 leading-tight mb-3"><?php echo htmlspecialchars($t['title']); ?></h4>
                                    <div class="flex items-center justify-center space-x-3">
                                        <span class="text-[9px] text-slate-400 font-black uppercase tracking-widest">Serial #<?php echo $t['id']; ?></span>
                                        <span class="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse"></span>
                                        <span class="text-[9px] text-green-500 font-black uppercase tracking-widest">Active bab</span>
                                    </div>
                                </div>

                                <div class="w-full space-y-4 relative z-10 mt-auto">
                                    <a href="modules.php?topic_id=<?php echo $t['id']; ?>" class="flex items-center justify-center space-x-3 w-full py-5 bg-slate-900 text-white font-black rounded-[2rem] hover:bg-blue-600 shadow-xl shadow-slate-900/10 hover:shadow-blue-500/20 transition-all transform active:scale-95 uppercase tracking-widest text-[10px] group/btn">
